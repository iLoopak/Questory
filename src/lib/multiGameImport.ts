...
export const playStationLibraryBookmarklet = String.raw`javascript:(async()=>{const norm=t=>(t||'').replace(/[™®©]/g,'').replace(/[^a-z0-9]+/gi,' ').replace(/\s+/g,' ').trim().toLowerCase();const clean=t=>(t||'').replace(/\s+/g,' ').trim();const visible=e=>!!(e&&e.getClientRects&&e.getClientRects().length&&getComputedStyle(e).visibility!=='hidden'&&getComputedStyle(e).display!=='none');const parseMeta=e=>{try{return JSON.parse(e?.getAttribute?.('data-telemetry-meta')||'{}')}catch{return{}}};const titleFrom=root=>{const titleEl=root.querySelector('[data-qa="collection-game-list-product#title"] span[id], [data-qa="collection-game-list-product#title"]');const title=clean(titleEl?.textContent);if(title)return title;const meta=parseMeta(root.querySelector('[data-qa="collection-game-list-product#store-link"]'));return clean(meta.titleName)||clean(root.querySelector('[data-qa="collection-game-list-product#game-art#image#image"]')?.getAttribute('alt'))};const platformsFrom=root=>{const out=[];root.querySelectorAll('[data-qa^="collection-game-list-product#platform-tags#tag"], [data-qa="collection-game-list-product#service-upsell#descriptorText"]').forEach(el=>{const value=clean(el.textContent);if(/\bPS5\b/i.test(value))out.push('PS5');if(/\bPS4\b/i.test(value))out.push('PS4');if(/PS\s*Plus/i.test(value))out.push('PS Plus')});return [...new Set(out)]};const bestImg=root=>{const img=root.querySelector('[data-qa="collection-game-list-product#game-art#image#image"]')||root.querySelector('[data-qa="collection-game-list-product#game-art#image#preview"]')||root.querySelector('img[src*="image.api.playstation.com"]')||root.querySelector('img');if(!img)return'';const candidates=(img.getAttribute('srcset')||'').split(',').map(part=>{const bits=part.trim().split(/\s+/);const url=bits[0];if(!url)return null;const scale=bits[1]&&bits[1].endsWith('x')?Number.parseFloat(bits[1]):1;const width=Number(new URL(url,img.src||location.href).searchParams.get('w')||0);return{url,score:width||scale*100}}).filter(Boolean);candidates.push({url:img.currentSrc||img.src||'',score:Number(new URL(img.currentSrc||img.src||location.href).searchParams.get('w')||0)});return candidates.filter(c=>c.url).sort((a,b)=>b.score-a.score)[0]?.url||''};const productRoots=[...document.querySelectorAll('[data-qa="collection-game-list-product"]')].filter(visible);const fallbackRoots=[...document.querySelectorAll('[data-qa="collection-game-list-product#game-art"]')].map(art=>art.closest('[data-qa="collection-game-list-product"],li,article,[role="listitem"]')).filter(Boolean).filter(visible);const roots=productRoots.length?productRoots:fallbackRoots;const pageNumber=Number((location.pathname.match(/\/recently-purchased\/(\d+)\/?$/)||[])[1]||'1');const seen=new Set();const games=[];for(const root of roots){const title=titleFrom(root);if(!title)continue;const platforms=platformsFrom(root);const usablePlatforms=platforms.length?platforms:['PlayStation'];const key=norm(title)+'|'+usablePlatforms.join(',');if(seen.has(key))continue;seen.add(key);games.push({title,platforms:usablePlatforms,coverUrl:bestImg(root),rawTitle:clean(root.textContent)||undefined})}const out={source:'playstation-library',version:1,pageUrl:location.href,pageNumber,exportedAt:new Date().toISOString(),games};try{await navigator.clipboard.writeText(JSON.stringify(out,null,2));alert('Questory: copied '+games.length+' PlayStation Library games from page '+pageNumber+'.')}catch(e){const ta=document.createElement('textarea');ta.value=JSON.stringify(out,null,2);ta.style='position:fixed;z-index:999999;top:20px;left:20px;width:80vw;height:70vh;background:#111;color:#fff;padding:16px;font:14px monospace;';document.body.appendChild(ta);ta.focus();ta.select();alert('Questory: found '+games.length+' games on page '+pageNumber+'. Copy them from the textarea.')}})()`;
