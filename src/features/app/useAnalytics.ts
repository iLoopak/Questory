...
import { trackAnalyticsEvent, type AnalyticsEventName, type TelemetryPropertiesFor } from '../../lib/analytics';
...
export function useAnalytics({ activeNavItem, activeQueuePlatforms, activeSettingsCategory, games, isAppReady, isOnboardingComplete, queuedCount }: UseAnalyticsParams) {
  ...
  // AS-17: the payload type is the event's own contract, so an undeclared property or an exact count
  // is a compile error at the call site rather than a rejected request in production.
  function trackMinimalAnalyticsEvent<Name extends AnalyticsEventName>(eventName: Name, properties: TelemetryPropertiesFor<Name>) { trackAnalyticsEvent(eventName, properties); }
  function trackSessionAnalyticsEvent<Name extends AnalyticsEventName>(eventName: Name, properties: TelemetryPropertiesFor<Name>) { const eventKey = `${eventName}:${JSON.stringify(properties)}`; if (trackedSessionEventsRef.current.has(eventKey)) return; trackedSessionEventsRef.current.add(eventKey); trackMinimalAnalyticsEvent(eventName, properties); }
