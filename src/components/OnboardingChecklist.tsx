...
import { getPersonalizedQuestShelfTitle } from '../lib/appPersonalization';
...
export function OnboardingChecklist({
  ...
  const onboardingQuestShelfTitle = getPersonalizedQuestShelfTitle(libraryOwnerNickname, null);
...
function SteamStep({ games, isComplete, onComplete, onContinue, onImportGames, onSkip, onSteamLibraryImported, onSteamProfileNameChange }: { games: Game[]; isComplete: boolean; onComplete: () => void; onContinue: () => void; onImportGames: (games: Game[]) => SteamOwnedImportSummary; onSkip: () => void; onSteamLibraryImported?: () => void; onSteamProfileNameChange?: (profileName: string) => void }) {
  ...
  async function importLibrary() {
    ...
    try {
      ...
      const mapped = mapSteamGamesToLocalGames(ownedGames, recentlyPlayedGames);
      const result = onImportGames(mapped);
      const importedCount = result.created + result.movedFromWishlist + result.updated;
      const skippedCount = result.skipped + result.failed;
      ...
      setImportResult({ status: 'success', importedCount, skippedCount });
  ...
  return (
    <div>
      <div className="mb-5 rounded-lg border border-skyglass/15 bg-ink-950/60 p-4">
        ...
        <p className="mt-1 text-sm text-slate-400">Questory reads your Steam profile to import games, playtime, and achievements. Your Steam account is never modified — Questory is read-only.</p>
      </div>
...
function RetroStep({ games, onComplete, onImportGames, onSkip }: { games: Game[]; onComplete: () => void; onImportGames: (games: Game[]) => SteamOwnedImportSummary; onSkip: () => void }) {
...
function FinishStep({
  ...
  return (
    <div className="rounded-2xl border border-mint/30 bg-mint/10 p-5 text-center shadow-glow">
      ...
      <h3 className="mt-4 text-3xl font-semibold text-white">Your Questory is ready.</h3>
      ...
      <div className="mx-auto mt-5 max-w-lg rounded-xl border border-white/10 bg-ink-950/60 p-4 text-left">
        <div className="text-xs font-semibold uppercase tracking-spread text-mint">Community Alpha</div>
        <p className="mt-1 text-sm text-slate-300">Help improve Questory by sharing anonymous usage stats — no game titles, notes, or personal data.</p>
