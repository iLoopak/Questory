...
export function ShelfIdentityEditor({ identity, onIdentityChange, shelfNamePlaceholder = 'Loopak', steamAvatarUrl, steamPersonaName }: ShelfIdentityEditorProps) {
  ...
  return <div className="space-y-4">
    <label className="block"><span className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Name / Nickname</span><input className="mt-2 h-11 w-full rounded-md border border-white/10 bg-ink-900 px-3 text-sm text-white outline-none focus:border-mint" maxLength={48} onChange={(event) => onIdentityChange({ ...identity, shelfName: event.target.value })} placeholder={shelfNamePlaceholder} value={identity.shelfName} /></label>
