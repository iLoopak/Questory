      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            {highlightLabel ? (
              <span className="rounded-full border border-amber-300/40 bg-amber-300 px-2 py-0.5 text-xs font-bold uppercase tracking-caps text-ink-950">
                {highlightLabel}
              </span>
            ) : null}
  ...
  return (
    <span className="grid h-full place-items-center bg-ink-700 px-3 text-center">
      <span>
        ...
        <span className="mt-2 block text-xs font-medium uppercase tracking-caps text-slate-500">{t('collection.noCover')}</span>
      </span>
    </span>
  );
}
